
/*  
    DOM -> DOCUMENT OBJECT MODEL
*/

// console.log(window.console)
// console.log("Me gusta el pancito")
// console.warn("Saca la mano de ahi maaamaaaaa, la loooooz!")
// console.error("404 - Not Found")

// console.log(document)

const c = console;
const d = document;

// c.log(d)
// c.log(d.doctype)
// c.log(d.documentElement)
// c.log(d.head)
// c.log(d.body)


// CONSULTA DE SELECTOR

// Consulta de selectores especificos
// d.getElementsByTagName("section") // Nombre de etiqueta      -> []
// c.log(d.getElementsByClassName("grupo")) // Nombre de clase  -> []
// c.log(d.getElementById("unico")) // Nombre de ID             -> element
// c.log(d.getElementsByName("viewport")) // Nombre de name     -> []

// Consulta de selectores universales
// c.log(d.querySelector("#unico")) //                          -> element
// c.log(d.querySelectorAll(".grupo"))                          -> []


const pokemons = [
    {
        img: "https://i0.wp.com/lordlibidan.com/wp-content/uploads/2019/03/Running-Pikachu-GIF.gif?resize=480%2C342&ssl=1",
        name: "Pikachu",
        type: "Electric",
        bgColor: "yellow"
    },
    {
        img: "https://upload.wikimedia.org/wikipedia/it/archive/4/43/20120529174523%21Bulbasaur.png",
        name: "Bulbasaur",
        type: "Plant/Venom",
        bgColor: "green"
    },
    {
        img: "https://freepngimg.com/thumb/pokemon/117717-charmander-png-image-high-quality-thumb.png",
        name: "Charmander",
        type: "Fire",
        bgColor: "orange"
    },
    {
        img: "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/8355c42b-bec4-426e-a0fa-f39523ea6c02/da74aa6-367253cd-ef9e-41fe-83df-64b857675335.png/v1/fill/w_848,h_720/vamo_a_calmarno_render_by_taringamemes_da74aa6-fullview.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..ZNF3N5MBYnZKSWpRRuWvSYFLbTvxacfR439C1L60lDk",
        name: "Squirtle",
        type: "Water",
        bgColor: "blue"
    },
    {
        img: "https://www.pokemon.com/static-assets/content-assets/cms2/img/pokedex/full/017.png",
        name: "Pidgeotto",
        type: "Flying/Normal",
        bgColor: "gold"
    },
]

const $cards = d.querySelector(".cards");

pokemons.forEach(pokemon => { // pokemon -> {} -> propiedades -> pokemon.type
    // Crear un div virtual
    const $div = d.createElement('div');
    $div.className = "card";
    $div.style.backgroundColor = pokemon.bgColor

    $div.innerHTML = `
        <img src=${pokemon.img} alt=${pokemon.name}>
        <h3>${pokemon.name}</h3>
        <p>${pokemon.type}</p>
    `

    $cards.appendChild($div)
})

